...
> The hostnames, emails, and IPs below are placeholders (`example.com`,
...
- **Homelab** (a private GitOps repo) — k3s, **Flux** GitOps, **SOPS** (age) secrets.
  Two relevant namespaces: `wardrowbe`, `ollama`. App images live in a **private**
  registry, pulled with a `dockerconfigjson` imagePullSecret in the `wardrowbe`
  namespace (created imperatively from a registry token; not stored in Git).
...
| Web UI ingress | `http://wardrowbe.local` (nginx, internal-only) | LAN DNS / hosts entry → node `192.0.2.10` |
...
- Web dev login (`DevCredentialsProvider`) requires **`DEV_MODE=true` on the
  ...
  by replacing `@`/`.` with `-`: `you@example.com` → **`you-example-com`**.
...
- **Origin**: `wardrowbe-mcp.example.com` → MCP `:8080` (bearer-protected).
- **Portal**: `wardrowbe-portal.example.com` → a **Cloudflare Access MCP server
  portal** that adds the **OAuth** the Connectors UI requires and injects the static
  bearer to the origin. Auth server: `https://<team>.cloudflareaccess.com`.
...
1. **Geo WAF rule** on the domain blocked everything outside one region → **403** for
   Cloudflare's MCP control plane *and* Anthropic's cloud (both out-of-region). Fix: a
   WAF **Skip** rule for the `wardrowbe-*` hostnames, above the geo rule.
...
3. **Portal `/mcp` 401 omitted `WWW-Authenticate`** → Claude couldn't discover OAuth and
   ...
   `WWW-Authenticate: Bearer resource_metadata="https://wardrowbe-portal.example.com/.well-known/oauth-protected-resource"`
...
Test image: a solid blue **linen** button-up (point collar, full placket, small
chest logo), flat product shot.
...
| **`qwen2.5vl:3b`** | 3.2 GB | **Winner.** Correct color, **no** white-collar/mannequin hallucination, **caught the logo**. Fits 8Gi. Errors: cotton vs linen (unavoidable from a photo), collar style slightly off, a "three buttons" miscount in one run. |
...
- The Ollama PVC keeps only the two active models (`qwen2.5vl:3b`,
  `qwen2.5:1.5b`) after cleanup.
- `--external-id` must match the web user's normalized `external_id`; pair it with
  `--external-email` to send the real address and avoid the email flap.
- The k8s manifests, Cloudflare bring-up steps, and these decisions live in the
  private homelab GitOps repo (`apps/wardrowbe*`, `build/`).
